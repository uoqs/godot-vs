...
In this setup, Godot Engine is built from source. Properties of the Visual Studio project generated by SCons are edited to keep engine configurations `target_release` and `editor` in a single project file. This allows switching configuration directly in Visual Studio and avoids excessive engine rebuilds.

The list of solution configurations no longer mirrors the list of default engine configurations, but instead follows the usual Visual Studio scheme. That way, more fine-tuned solution configurations can be added if necessary.

Optionally, Hot Reload can be enabled.
...
5. Open the **Reference Manager** and select the **game-cpp** project. Add a reference to the **godot-cpp** project.
...
1. To enable Hot Reload for Godot Editor, replace option `/Zi` with `/ZI` in the file `godot\SConstruct`
    
2. To enable Hot Reload for the game code, replace option `/Zi` with `/ZI` in the file `godot‑cpp\tools\target.py`
...
2. Because the Godot project doesn’t yet exist, a Godot **Project Manager** will launch instead of the editor.

3. Create a new Godot project in the `game` directory. You will see a warning stating that the selected directory is not empty, and the existing files will be imported as project resources. Confirm and continue.

4. After the Godot project is created, the editor will launch as a separate process. Use **Attach to Process…** in Visual Studio to be able to debug the code and use hot reload.
